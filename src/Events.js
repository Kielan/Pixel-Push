...
const Events = (() => {
   ...
   * 
   * @param {*} document the property of the browser window object
   ...
   */
   function on(event, elementId, functionCallback, ...args) {
      ...
      const element = Util.getElement(document, elementId);
  ...
  function onChildren(event, parentElement, functionCallback, ...args) {
      parentElement = Util.getElement(document, parentElement);
